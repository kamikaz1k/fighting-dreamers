export type MoveButton = "weak" | "strong";
export type MoveDirection = "neutral" | "forward" | "back" | "up" | "down";
export type MoveContext = "ground" | "air";
export type MoveRole =
  | "quick-poke"
  | "long-poke"
  | "quick-pop-up"
  | "heavy-pop-up"
  | "close-low"
  | "air-poke"
  | "air-control"
  | "placeholder";

export type MoveDefinition = {
  id: string;
  button: MoveButton;
  direction: MoveDirection;
  context: MoveContext;
  role: MoveRole;
  startupFrames: number;
  activeFrames: number;
  recoveryFrames: number;
  damage: number;
  knockback: { x: number; y: number };
  hitbox: { x: number; y: number; width: number; height: number };
  shieldDamage: number;
  hitstopFrames: number;
  movementMultiplier?: number;
};

export const moveDefinitions: Record<string, MoveDefinition> = {
  groundNeutralWeak: {
    id: "groundNeutralWeak",
    button: "weak",
    direction: "neutral",
    context: "ground",
    role: "quick-poke",
    startupFrames: 5,
    activeFrames: 4,
    recoveryFrames: 12,
    damage: 6,
    knockback: { x: 320, y: -80 },
    hitbox: { x: 24, y: -78, width: 38, height: 24 },
    shieldDamage: 12,
    hitstopFrames: 4,
    movementMultiplier: 0.45,
  },
  groundForwardWeak: {
    id: "groundForwardWeak",
    button: "weak",
    direction: "forward",
    context: "ground",
    role: "long-poke",
    startupFrames: 7,
    activeFrames: 4,
    recoveryFrames: 14,
    damage: 8,
    knockback: { x: 390, y: -70 },
    hitbox: { x: 28, y: -76, width: 48, height: 24 },
    shieldDamage: 15,
    hitstopFrames: 5,
    movementMultiplier: 0.4,
  },
  groundBackWeak: {
    id: "groundBackWeak",
    button: "weak",
    direction: "back",
    context: "ground",
    role: "long-poke",
    startupFrames: 7,
    activeFrames: 4,
    recoveryFrames: 14,
    damage: 8,
    knockback: { x: 360, y: -90 },
    hitbox: { x: 22, y: -76, width: 46, height: 24 },
    shieldDamage: 15,
    hitstopFrames: 5,
    movementMultiplier: 0.4,
  },
  groundUpWeak: {
    id: "groundUpWeak",
    button: "weak",
    direction: "up",
    context: "ground",
    role: "quick-pop-up",
    startupFrames: 6,
    activeFrames: 5,
    recoveryFrames: 15,
    damage: 7,
    knockback: { x: 120, y: -455 },
    hitbox: { x: -14, y: -124, width: 44, height: 48 },
    shieldDamage: 14,
    hitstopFrames: 5,
    movementMultiplier: 0.5,
  },
  groundDownWeak: {
    id: "groundDownWeak",
    button: "weak",
    direction: "down",
    context: "ground",
    role: "close-low",
    startupFrames: 6,
    activeFrames: 4,
    recoveryFrames: 13,
    damage: 6,
    knockback: { x: 260, y: 160 },
    hitbox: { x: 18, y: -38, width: 42, height: 26 },
    shieldDamage: 13,
    hitstopFrames: 4,
    movementMultiplier: 0.45,
  },
  groundNeutralStrong: {
    id: "groundNeutralStrong",
    button: "strong",
    direction: "neutral",
    context: "ground",
    role: "long-poke",
    startupFrames: 8,
    activeFrames: 5,
    recoveryFrames: 18,
    damage: 9,
    knockback: { x: 440, y: -110 },
    hitbox: { x: 22, y: -58, width: 54, height: 28 },
    shieldDamage: 18,
    hitstopFrames: 6,
    movementMultiplier: 0.35,
  },
  groundForwardStrong: {
    id: "groundForwardStrong",
    button: "strong",
    direction: "forward",
    context: "ground",
    role: "long-poke",
    startupFrames: 10,
    activeFrames: 5,
    recoveryFrames: 20,
    damage: 11,
    knockback: { x: 560, y: -120 },
    hitbox: { x: 30, y: -62, width: 66, height: 30 },
    shieldDamage: 22,
    hitstopFrames: 7,
    movementMultiplier: 0.28,
  },
  groundBackStrong: {
    id: "groundBackStrong",
    button: "strong",
    direction: "back",
    context: "ground",
    role: "long-poke",
    startupFrames: 11,
    activeFrames: 5,
    recoveryFrames: 21,
    damage: 11,
    knockback: { x: 520, y: -140 },
    hitbox: { x: 24, y: -62, width: 60, height: 30 },
    shieldDamage: 22,
    hitstopFrames: 7,
    movementMultiplier: 0.28,
  },
  groundUpStrong: {
    id: "groundUpStrong",
    button: "strong",
    direction: "up",
    context: "ground",
    role: "heavy-pop-up",
    startupFrames: 10,
    activeFrames: 6,
    recoveryFrames: 22,
    damage: 10,
    knockback: { x: 120, y: -610 },
    hitbox: { x: -16, y: -132, width: 50, height: 58 },
    shieldDamage: 20,
    hitstopFrames: 7,
    movementMultiplier: 0.3,
  },
  groundDownStrong: {
    id: "groundDownStrong",
    button: "strong",
    direction: "down",
    context: "ground",
    role: "close-low",
    startupFrames: 9,
    activeFrames: 5,
    recoveryFrames: 18,
    damage: 9,
    knockback: { x: 360, y: 220 },
    hitbox: { x: 16, y: -32, width: 58, height: 30 },
    shieldDamage: 19,
    hitstopFrames: 6,
    movementMultiplier: 0.35,
  },
  airNeutralWeak: {
    id: "airNeutralWeak",
    button: "weak",
    direction: "neutral",
    context: "air",
    role: "air-poke",
    startupFrames: 5,
    activeFrames: 5,
    recoveryFrames: 14,
    damage: 6,
    knockback: { x: 270, y: -80 },
    hitbox: { x: 18, y: -76, width: 46, height: 34 },
    shieldDamage: 12,
    hitstopFrames: 4,
    movementMultiplier: 0.72,
  },
  airForwardWeak: {
    id: "airForwardWeak",
    button: "weak",
    direction: "forward",
    context: "air",
    role: "air-control",
    startupFrames: 7,
    activeFrames: 5,
    recoveryFrames: 16,
    damage: 7,
    knockback: { x: 360, y: -80 },
    hitbox: { x: 26, y: -72, width: 54, height: 30 },
    shieldDamage: 14,
    hitstopFrames: 5,
    movementMultiplier: 0.65,
  },
  airBackWeak: {
    id: "airBackWeak",
    button: "weak",
    direction: "back",
    context: "air",
    role: "air-control",
    startupFrames: 7,
    activeFrames: 5,
    recoveryFrames: 16,
    damage: 7,
    knockback: { x: 330, y: -110 },
    hitbox: { x: 20, y: -72, width: 50, height: 30 },
    shieldDamage: 14,
    hitstopFrames: 5,
    movementMultiplier: 0.65,
  },
  airUpWeak: {
    id: "airUpWeak",
    button: "weak",
    direction: "up",
    context: "air",
    role: "quick-pop-up",
    startupFrames: 6,
    activeFrames: 5,
    recoveryFrames: 17,
    damage: 7,
    knockback: { x: 90, y: -420 },
    hitbox: { x: -14, y: -120, width: 44, height: 48 },
    shieldDamage: 14,
    hitstopFrames: 5,
    movementMultiplier: 0.7,
  },
  airDownWeak: {
    id: "airDownWeak",
    button: "weak",
    direction: "down",
    context: "air",
    role: "air-control",
    startupFrames: 7,
    activeFrames: 5,
    recoveryFrames: 18,
    damage: 7,
    knockback: { x: 180, y: 310 },
    hitbox: { x: 0, y: -40, width: 42, height: 44 },
    shieldDamage: 14,
    hitstopFrames: 5,
    movementMultiplier: 0.62,
  },
  airNeutralStrong: {
    id: "airNeutralStrong",
    button: "strong",
    direction: "neutral",
    context: "air",
    role: "air-poke",
    startupFrames: 9,
    activeFrames: 6,
    recoveryFrames: 22,
    damage: 10,
    knockback: { x: 430, y: -120 },
    hitbox: { x: 18, y: -70, width: 62, height: 36 },
    shieldDamage: 20,
    hitstopFrames: 6,
    movementMultiplier: 0.5,
  },
  airForwardStrong: {
    id: "airForwardStrong",
    button: "strong",
    direction: "forward",
    context: "air",
    role: "air-control",
    startupFrames: 11,
    activeFrames: 6,
    recoveryFrames: 24,
    damage: 11,
    knockback: { x: 540, y: -130 },
    hitbox: { x: 30, y: -66, width: 68, height: 34 },
    shieldDamage: 22,
    hitstopFrames: 7,
    movementMultiplier: 0.45,
  },
  airBackStrong: {
    id: "airBackStrong",
    button: "strong",
    direction: "back",
    context: "air",
    role: "air-control",
    startupFrames: 11,
    activeFrames: 6,
    recoveryFrames: 24,
    damage: 11,
    knockback: { x: 500, y: -140 },
    hitbox: { x: 24, y: -66, width: 64, height: 34 },
    shieldDamage: 22,
    hitstopFrames: 7,
    movementMultiplier: 0.45,
  },
  airUpStrong: {
    id: "airUpStrong",
    button: "strong",
    direction: "up",
    context: "air",
    role: "heavy-pop-up",
    startupFrames: 10,
    activeFrames: 6,
    recoveryFrames: 24,
    damage: 10,
    knockback: { x: 90, y: -560 },
    hitbox: { x: -18, y: -132, width: 54, height: 58 },
    shieldDamage: 20,
    hitstopFrames: 7,
    movementMultiplier: 0.48,
  },
  airDownStrong: {
    id: "airDownStrong",
    button: "strong",
    direction: "down",
    context: "air",
    role: "air-control",
    startupFrames: 12,
    activeFrames: 6,
    recoveryFrames: 26,
    damage: 11,
    knockback: { x: 220, y: 460 },
    hitbox: { x: -4, y: -36, width: 54, height: 48 },
    shieldDamage: 22,
    hitstopFrames: 7,
    movementMultiplier: 0.42,
  },
};
